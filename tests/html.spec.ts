...
describe('Inputs have valid attributes', () => {
  ...
  test('A toggle for air conditioner/heating system', () => {
    const input = document.querySelector<HTMLInputElement>(
      'input#ac[type="checkbox"]',
    );
    expect(input).not.toBeNull();
  });

  test('A wheel size selection with 19" and 21".', () => {
    const input19 = document.querySelector<HTMLInputElement>(
      'input[type="radio"][value="19"]',
    );
    const input21 = document.querySelector<HTMLInputElement>(
...
describe('HTML template is valid', () => {
  ...
  test('Page has valid Main content', () => {
    ...
    expect(
      document.querySelector('.intro__title._observed-fade-in'),
    ).not.toBeNull();

    expect(
      document.querySelector('.intro__subtitle._observed-fade-in'),
    ).not.toBeNull();
    ...
    expect(
      document.querySelectorAll(`.calculator__fields .calculator__field-group`)
        .length,
    ).toBe(3);
  });
  ...
  test('Page has a valid footer', () => {
    expect(
      document.querySelectorAll('.footer .footer__content .footer__group')
        .length,
    ).toBe(4);
  });
  ...
  test('Page has a cursor animation overlay', () => {
    expect(
      document.querySelectorAll(
        '.overlay .cursor .cursor__circle[aria-hidden="true"]',
      ),
    ).not.toBeNull();
  });
});
